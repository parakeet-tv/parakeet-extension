...
This directory is special because it contains a full-blown Svelte application which was created using the TypeScript-based Svelte template. As a result, `webview-ui` contains its own `package.json`, `node_modules`, `tsconfig.json`, and so on––separate from the `parakeet` extension in the root directory.
